...
const serverDownloadUrl = 'https://www.eclipse.org/downloads/download.php?file=/jdtls/milestones/1.45.0/jdt-language-server-1.45.0-202502271238.tar.gz'
const serverLauncher = '/plugins/org.eclipse.equinox.launcher_1.6.1000.v20250131-0606.jar'
const minJavaRuntime = 21
...
class JavaLanguageClient extends AutoLanguageClient {
  ...
  getJavaVersionFromOutput (output) {
    const match = output.match(/ version "(\d+(?:\.\d+)*)(_\d+)?(?:-\w+)?"/)
    if (match === null || match.length == 0) return null;

    const semvar = match[1].split('.');
    if (semvar[0] == "1")
      return Number(semvar[1]);
    return Number(semvar[0]);
  }
  ...
  installServer (serverHome) {
    ...
    const provideInstallStatus = (bytesDone, percent) => {
      this.updateInstallStatus(`downloading ${Math.floor(bytesDone / bytesToMegabytes)} MB (${percent}% done)`)
    }
    return this.fileExists(serverHome)
      .then(doesExist => { if (!doesExist) fs.mkdirSync(serverHome, { recursive: true }) })
      .then(() => DownloadFile(serverDownloadUrl, localFileName, provideInstallStatus))
